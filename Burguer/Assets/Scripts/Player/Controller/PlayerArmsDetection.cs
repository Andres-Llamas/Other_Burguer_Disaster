...
namespace Player
{
    public class PlayerArmsDetection : MonoBehaviour
    {
        ...
        void RecalculateRayDistance()// this is to increase the ray lenght when player is watching downwards or upwards
        {
        ...
        void DetectAnObject() // to detect a dinamic object
        {
  ...
  //          if (objectOnHand.gameObject.tag != "Plate")
            ...
            objectOnHand.transform.position = hand.position; // move ohterObject to the hand position
            objectOnHand.transform.parent = hand;
            objectOnHand.transform.localScale = Vector3.one * otherObjectComponents.ObjectSizeOnHand; // modify object size to fit the hand size
            objectOnHand.transform.rotation = Quaternion.Euler(0, 0, 0);// reset rotation
        }
